...
class Augmentor(nn.Module):
    ...
    def forward(self, points):
        '''
        Args:
            points: a cuda tensor of shape [B, N, 3]
        return:
            aug_pc: a cuda tensor of shape [B, N, 3],
            aug_stats: a dictionary of transformation applied
        '''
        aug_stats = {}
        batchsize = points.shape[0]
        # points = points.transpose(2, 1)
        noise = torch.randn(batchsize, 9).to(self.device)
        ...
        tensor_0 = torch.zeros(batchsize).to(self.device)
        tensor_1 = torch.ones(batchsize).to(self.device)
        p1 = random.random()
        p2 = random.random()
        p3 = random.random()
        p4 = random.random()
        ...
        if self.apply_noise:
            ...
            if p4 > thres:
                point_shift = (torch.rand(batchsize, 8096).to(self.device) * 0.002 + noise_squeeze)[:, :, None]
                points = points + point_shift
        # points = points.transpose(2, 1)
        return points, aug_stats
